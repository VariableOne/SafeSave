...
export default class StudentsController {
  ...
  public async registerProcess({ request, view }: HttpContext) {
    ...
    if (!/^\d+$/.test(matrikelnummer)) {
      return view.render('pages/registration', {
        currentPath,
        error: 'Matrikeknummer dürfen nur Zahlen beinhalten.',
      })
    }
    ...
    if (password !== confirmPassword) {
      ...
      return view.render('pages/registration', {
        currentPath,
        error: 'Passwort und Bestätigungspasswort stimmen nicht überein!',
      })
    }
    ...
    if (
      ...
      return view.render('pages/registration', {
        currentPath,
        error:
          'Das Passwort muss mindestens 8 Zeichen lang sein und mindestens 2 Großbuchstaben sowie 2 Zahlen enthalten!',
      })
    }
  ...
  public async checkDataOfStudent({ view, request }: HttpContext) {
    ...
    if (!student) {
      return view.render('pages/resetPassword', {
        currentPath,
        error: 'Email oder Matrikelnummer ist falsch.',
      })
    }
  ...
  public async setNewPassword({ request, view }: HttpContext) {
    ...
    if (password !== confirmPassword) {
      console.log('Passwords do not match')
      return view.render('pages/newPassword', {
        currentPath,
        error: 'Passwort und Bestätigungspasswort stimmen nicht überein!',
      })
    }
